...
async function getCountriesData(){
    try{
        var countriesDataRequest = await fetch("https://restcountries.com/v3.1/all")
...
getCountriesData().then((countriesData)=>{
    ...
    for(var i=0;i<countriesData.length;i++){
        ...
        countryCards[i].innerHTML = `<p class="bg-dark text-white"> <b> ${countriesData[i].name.common} </b> </p> 
        <img class="border flag_img card-img-top" src="${countriesData[i].flags.png}" alt="Flag">`
        ...
        if(countriesData[i].cca3==="") {countriesData[i].cca3="-"}
        ...
        countryCodeElem.innerHTML = `Country Code: <b>${countriesData[i].cca3}`
